...
{ 
  ...
  CurrentFrameBuffer = mymalloc(SRAMEX,  800 * 480 * 2 );
...
void LCD_LayerInit(void)
{
  ...
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((LCD_PIXEL_WIDTH * 2) + 2);
  ...
  /* 配置本层的显存首地址 */    
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = CurrentFrameBuffer;
  ...
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = CurrentFrameBuffer + BUFFER_OFFSET;
...
void LCD_SetLayer(uint32_t Layerx)
{
  ...
  {
    CurrentFrameBuffer = CurrentFrameBuffer + BUFFER_OFFSET;
 ...
 */
 ...
 LTDC_Layer_InitStruct.LTDC_CFBStartAdress = (uint32_t)CurrentFrameBuffer;
...
// LTDC_Layer_InitStruct.LTDC_CFBStartAdress = (uint32_t)CurrentFrameBuffer + BUFFER_OFFSET;
...
void LCD_SetLayer(uint32_t Layerx)
{
 ...
 {
   CurrentFrameBuffer =(uint32_t)CurrentFrameBuffer + BUFFER_OFFSET;
   CurrentLayer = LCD_FOREGROUND_LAYER;
 }
}
...
void LCD_Clear(uint16_t Color)
{
 ...
 DMA2D_InitStruct.DMA2D_OutputMemoryAdd = (uint32_t)CurrentFrameBuffer;
 DMA2D_InitStruct.DMA2D_OutputOffset = 0;
 DMA2D_InitStruct.DMA2D_NumberOfLine = LCD_PIXEL_HEIGHT;
 DMA2D_InitStruct.DMA2D_PixelPerLine = LCD_PIXEL_WIDTH;
 DMA2D_Init(&DMA2D_InitStruct);
...
uint32_t LCD_SetCursor(uint16_t Xpos, uint16_t Ypos)
{
 return (uint32_t)CurrentFrameBuffer + 2*(Xpos + (LCD_PIXEL_WIDTH*Ypos));
}
