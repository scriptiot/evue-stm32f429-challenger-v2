...
int8_t js_page_sm_init(evm_t * e, js_page_sm_t * sm, evm_val_t * object, evm_val_t * js_res){
    ...
    sm->js_app_dir =  (char*)evm_2_string(evm_prop_get(e, object, APP_DIR, 0));
    sm->uri = NULL;
...
int8_t js_page_sm_bind_uri(js_page_sm_t * sm, evm_val_t * js_res){
    ...
    sm->uri = (char*)evm_2_string(uriValue);
    if (sm->uri == NULL) {
        evm_print("statemachine init failed as uri is invalid.\n");
        return 0;
    }
...
void js_page_sm_render_page(js_page_sm_t * sm){
    ...
    sm->page->hml_object = evm_module_load(e, router_temp_path, 0);
    ...
    sm->page->css_object = evm_module_load(e, router_temp_path, EVM_DO_NOT_REFRESH_MODULE);

    hml_page_init(e, sm->page);
    evm_val_t * appdir = evm_prop_get(js_global_e, &sm->object, APP_DIR, EVM_DO_NOT_REFRESH_MODULE);
    sm->page->init(js_global_e, NULL, 1, appdir);
    hml_run(e, sm->page);

    evm_print("load view model\r\n");
...
evm_val_t js_page_sm_show_page(js_page_sm_t * sm){
    ...
    evm_heap_gc(e);
    evm_print("show page\n");
    evm_print("Memory usage: %d / %d bytes\r\n", js_global_e->heap->usage, js_global_e->heap->size);
