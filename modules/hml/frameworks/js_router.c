...
evm_val_t js_router_add(js_router_t * router, evm_val_t * object, int index){
    ...
    char * uri = (char*)evm_2_string(uriValue);
    ...
    evm_prop_append(js_global_e, object, (char*)APP_DIR, &dirVal);
    ...
    evm_list_set(js_global_e, js_fwk_index_stacks, index, newSm->page->index_object);
    evm_list_set(js_global_e, js_fwk_hml_stacks, index, newSm->page->hml_object);
    evm_list_set(js_global_e, js_fwk_css_stacks, index, newSm->page->css_object);
