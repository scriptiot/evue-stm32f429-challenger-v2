...
static evm_hml_property_reg_t * hml_object_style_properties;

int hml_objects_len;
int hml_object_style_properties_len;

int __hml_value_check_type(evm_hml_property_reg_t * value, evm_val_t * v){
    if( !value ) return 0;
    if( value->type & EVM_HML_ANY ) return 1;
    if( (value->type & EVM_HML_INT) && evm_is_number(v) ) return 1;
    if( (value->type & EVM_HML_DOUBLE) && evm_is_number(v) ) return 1;
    if( (value->type & EVM_HML_STRING) && evm_is_string(v) ) return 1;
    if( (value->type & EVM_HML_BOOLEAN) && evm_is_boolean(v) ) return 1;
    return 0;
}

evm_hml_property_reg_t * __hml_value_get(hml_object_t * object, const char * name){
    hml_object_t * parent = object;
    while(parent){
        for(int i = 0; i < parent->properties_len; i++){
            evm_hml_property_reg_t * o = parent->properties + i;
...
static evm_val_t propertyChanged(evm_t * e, evm_val_t * p, int argc, evm_val_t * v){
    ...
    while(bindings){
        if( name == bindings->src_name ){
            ...
            for(int i = 0; i < hml_object->properties_len; i++){
                evm_hml_property_reg_t * value = hml_object->properties + i;
    ...
    if( !triggered ){
        ...
        while(local_func_bindings){
            ...
            if(ret != local_func_bindings->ret){
                ...
                hml_object_t * hml_obj = (hml_object_t*)evm_object_get_ext_data(&local_func_bindings->obj);
                evm_val_t * value = evm_prop_get(e, &local_func_bindings->obj, "value", 0);
                evm_hml_property_reg_t * hml_value = __hml_value_get(hml_obj, "text");
                if( hml_value) {
                    if(hml_value->api) {
 ...
 * @return 长度
 */
int __hml_value_reg_len(evm_hml_property_reg_t * regs){
    if(regs == NULL) return 0;
    int i = 0;
    while(1){
        if( regs[i].name == NULL ) return i;
...
int evm_hml_object_register(evm_t * e, evm_hml_object_reg_t * regs){
    ...
    for(int i = 0; i < objects_len; i++){
        ...
        int properties_len = __hml_value_reg_len(regs[i].properties);
        if( properties_len ){
            hml_objects[i].properties_len = properties_len;
            hml_objects[i].properties = (evm_hml_property_reg_t*)evm_malloc(properties_len * sizeof(evm_hml_property_reg_t));
            if(!hml_objects[i].properties) return ec_memory;
            memset(hml_objects[i].properties, 0, properties_len * sizeof(evm_hml_property_reg_t));
            for(int j = 0; j < properties_len; j++){
                hml_objects[i].properties[j].api = regs[i].properties[j].api;
                hml_objects[i].properties[j].name = regs[i].properties[j].name;
                hml_objects[i].properties[j].type = regs[i].properties[j].type;
            }
        }
    }
    return ec_ok;
...
int evm_hml_style_register(evm_t *e, evm_hml_property_reg_t *regs)
{
    EVM_UNUSED(e);
    if( !regs ) return ec_err;
    hml_object_style_properties_len = __hml_value_reg_len(regs);
    hml_object_style_properties = (evm_hml_property_reg_t*)evm_malloc(hml_object_style_properties_len * sizeof(evm_hml_property_reg_t));
    memset(hml_object_style_properties, 0, hml_object_style_properties_len * sizeof(evm_hml_property_reg_t));
    for(int i=0; i < hml_object_style_properties_len; i++){
        hml_object_style_properties[i].type = regs[i].type;
        hml_object_style_properties[i].name = regs[i].name;
        hml_object_style_properties[i].api = regs[i].api;
        hml_object_style_properties[i].key = regs[i].key;
    }
    return ec_ok;
}
...
void * __hml_object_create(evm_t * e, hml_page_t * page, evm_val_t * parent, const char* name, evm_val_t * hml, evm_val_t viewmodel, evm_val_t * css_label, evm_val_t * css_class, evm_val_t * css_id, evm_val_t * keyframes){
    ...
    if( parent ){
        ...
        evm_hml_property_reg_t * value = __hml_value_get(parent_hml_obj, name);
        if(value && value->api){
            evm_native_fn api = value->api;
            api(e, obj, 1, obj);
        }
...
evm_native_fn hml_display_set_style;
evm_native_fn hml_object_set_style;
...
//    native_print(e, NULL, 1, css_keyframes);
    ...
    hml_display_set_style(e, NULL, 1, css_label);
...
evm_val_t hml_object_set_style_1(evm_t * e, evm_val_t *hml, int argc, evm_val_t * style_obj){
    int property_len = evm_prop_len(style_obj);
    native_print(e, NULL, 1, style_obj);
    for (int i=0; i < property_len; i++){
        uint32_t key = evm_prop_get_key_by_index(e, style_obj, i);
        for (int j=0; j < hml_object_style_properties_len; j++){
            uint32_t style_key = hml_object_style_properties[j].key;
            if (style_key == key){
                printf("=======%s=====%d=========\n", evm_string_get(e, key), key);
                evm_native_fn  api = hml_object_style_properties[j].api;
                api(e, hml, 1, evm_prop_get_by_key(e, style_obj, key, 0));
            }
        }
    }
	return EVM_VAL_UNDEFINED;
...
//            native_print(e, NULL, 1, css_id_properties);
            merge_obj(e, style_obj, css_id_properties);
        }
    }
    if (style){
        merge_obj(e, style_obj, style);
    }

    evm_val_t * animation_name = evm_prop_get(e, style_obj, "animation-name", 0);
    if (animation_name && evm_is_string(animation_name)){
        ...
        if (animation_keyframes && evm_is_object(animation_keyframes)){
            ...
            if (from_key == to_key){
                if (evm_prop_get(e, style_obj,  "animation-property", 0)){
                    evm_prop_set_value(e, style_obj, "animation-property", *(evm_string_pool_get(e, from_key)));
                }else{
                    evm_prop_append(e, style_obj, "animation-property", evm_string_pool_get(e, from_key));
                }
                if (evm_prop_get(e, style_obj,  "animation-property-from", 0)){
                    evm_prop_set_value(e, style_obj, "animation-property-from", *(evm_prop_get_by_key(e, from, from_key, 0)));
                }else{
                    evm_prop_append(e, style_obj, "animation-property-from", evm_prop_get_by_key(e, from, from_key, 0));
                }
                if (evm_prop_get(e, style_obj,  "animation-property-to", 0)){
                    evm_prop_set_value(e, style_obj, "animation-property-to", *(evm_prop_get_by_key(e, to, from_key, 0)));
                }else{
                    evm_prop_append(e, style_obj, "animation-property-to", evm_prop_get_by_key(e, to, from_key, 0));
                }
            }
        }
    }
    hml_object_set_style_1(e, hml, 0, style_obj);
    hml_object_set_style(e, hml, 0, style_obj);
    return ec_ok;
}

evm_val_t hml_eval(evm_t * e, evm_val_t * p, int argc, evm_val_t * v){
    if( argc > 0 ){
        char * string = (char*)evm_2_string(v);
        evm_val_t * data_value = evm_prop_get(e, &current_page->index_object, "data", 0);
...
evm_err_t hml_module(evm_t * e,
    ...
    hml_display_set_style = set_display_style;
    hml_object_set_style = set_style;
    hml_show = show;
    hml_hide = hide;
